...
def convert_examples_to_features(examples, tokenizer, max_seq_length, split_num,
    ...
    for example_index, example in enumerate(examples):
        ...
        skip_len = int((max_seq_length - len(ending_tokens) - 3)/2)
        ...
        for i in range(split_num):
            if len(context_tokens) < (max_seq_length - len(ending_tokens) - 3):
                context_tokens_choice = context_tokens
            else:
                context_tokens_choice = context_tokens[:skip_len] + context_tokens[-skip_len:]
...
def main():
    ...
    if args.predict_eval:
        ...
        gc.collect()
        args.do_train = False
        model = BertForSequenceClassification_last2embedding_cls.from_pretrained(os.path.join(args.output_dir, "pytorch_model.bin"), args,
                                                              config=config)
        if args.fp16:
            model.half()
        model.to(device)
